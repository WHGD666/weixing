...
- `submit/v2` 已完成 Windows 和 Docker 全量复核，并以 `trial-v2.0` 完成官方试运行；结果见 `SUB002_trial_2750.md`。

## 8. 官方结果确认

官方提交 `2750`（`trial-v2.0`）状态为 `ACCEPTED`，任务已完成并计入成绩，综合分为 `84.8310`，排名第 42。相对 v1 的 `84.3313`，分数提升 `0.4997`。

平台隐藏测试结果与本地方向一致：ship 和 aircraft 指标完全不变，vehicle FDR 从 `0.274611` 降到 `0.244565`，vehicle Recall 从 `0.939597` 降到 `0.932886`，平均推理时间从 `1.837167s` 降到 `1.791s`。因此 INF009 被官方结果**正向验证**，但收益属于小幅改善，不能据此推断后续所有数据集都会同幅度提升。

本轮已完成从固定验证集候选选择、Docker 全量复核到官方试运行的闭环；`submit/v1` 保留为回滚基线，`submit/v2` 作为当前最佳提交候选归档。
