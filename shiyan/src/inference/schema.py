"""Validation for the official Docker result.json contract."""

import math
from collections.abc import Iterable, Mapping
from pathlib import Path

from .labels import CLASS_NAMES, CLASS_COUNT
...
def validate_result_document(
    ...
    expected_file_names: Iterable[str] | None = None,
) -> None:
    """Raise ValueError when an official result document is malformed."""
    ...
    for image_index, entry in enumerate(entries):
        ...
        if not isinstance(image_id, str) or not image_id:
            raise ValueError(f"images[{image_index}].image_id must be a non-empty string")
        if not isinstance(file_name, str) or not file_name:
            raise ValueError(f"images[{image_index}].file_name must be a non-empty string")
        if image_id != Path(file_name).stem:
        ...
        width = _number(entry.get("width"), f"images[{image_index}].width")
        height = _number(entry.get("height"), f"images[{image_index}].height")
        if width <= 0 or height <= 0:
            raise ValueError(f"image dimensions must be positive for {file_name}")
        timestamp = entry.get("run_end_timestamp")
        if isinstance(timestamp, bool) or not isinstance(timestamp, int) or timestamp < 0:
            raise ValueError(f"run_end_timestamp must be a non-negative integer for {file_name}")
        ...
        for object_index, item in enumerate(objects):
            ...
            if item.get("category_name") != CLASS_NAMES[category_id]:
                raise ValueError(f"category_name mismatch for {file_name}, category_id={category_id}")
            ...
            coordinates = [_number(value, f"bbox for {file_name}") for value in bbox]
    ...
    if expected is not None and actual_names != expected:
        raise ValueError(
            "result image order or coverage mismatch: "
            f"expected {len(expected)} files, received {len(actual_names)}"
        )
