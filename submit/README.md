...
当前 `v1` 已完成模型、推理入口、Linux 运行环境、Docker 镜像、ACR 推送和官方试运行。平台提交 `2551` 显示 `ACCEPTED`，综合分 `84.3313`，排名第 42。模型、测试图片和测试输出仍保持本地，不进入 GitHub。

## 版本规则

- `v1`：比赛方原始数据、原始标签、`EXP001` 基线，不进行人工修标；已完成官方试运行并归档结果。
