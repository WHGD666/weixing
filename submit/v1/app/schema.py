"""Minimal validation for the official result.json contract."""
...
def validate_result_document(document: Mapping[str, object], expected_file_names: Iterable[str]) -> None:
    ...
    expected = list(expected_file_names)
    ...
    for image_index, entry in enumerate(entries):
        ...
        for item in objects:
            if not isinstance(item, Mapping):
                raise ValueError(f"object must be an object for {file_name}")
            ...
            if item.get("category_name") != CLASS_NAMES[category_id]:
                raise ValueError(f"category_name mismatch for {file_name}")
            ...
            if not (0.0 <= x1 <= x2 <= width and 0.0 <= y1 <= y2 <= height):
                raise ValueError(f"bbox outside image bounds for {file_name}")
